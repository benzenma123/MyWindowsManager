...
int main() {
    ...
    // Lắng nghe thêm sự kiện ButtonPress và MotionNotify
    XSelectInput(display, root_window, SubstructureNotifyMask | SubstructureRedirectMask | KeyPressMask | ButtonPressMask);

    XSetWindowAttributes attributes;
    attributes.event_mask = SubstructureNotifyMask | SubstructureRedirectMask | KeyPressMask | ButtonPressMask;
    ...
    // Thêm phím Super + Q để đóng cửa sổ
    ...
    while (true) {
        ...
        switch (event.type) {
            case CreateNotify:
                ...
                // Lắng nghe sự kiện trên các cửa sổ con
